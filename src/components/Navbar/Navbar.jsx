...
export default function Navbar() {
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-8 flex items-center justify-between gap-6">
                ...
                <a href={href} onClick={e => go(e, href)}
                   className={`relative px-4 py-2 text-sm font-medium rounded-full no-underline transition-all duration-300
                     ${active === id
                       ...
                       : 'text-violet-950 hover:text-violet-600 hover:bg-violet-50/70'
                     }`}>
                  {label}
                  {active === id && (
                    <span className="absolute bottom-1 left-1/2 -translate-x-1/2 w-1/2 h-0.5
                                     rounded-full bg-gradient-to-r from-violet-600 to-pink-500" />
                  )}
                </a>
              </li>
            );
        ...
        <a href="#contact" onClick={e => go(e, '#contact')}
           className="hidden lg:flex items-center gap-2 px-6 py-2.5 flex-shrink-0 whitespace-nowrap no-underline
                      ...
                      shadow-lg shadow-violet-300/50
                      transition-all duration-300 hover:-translate-y-0.5 hover:scale-[1.04] hover:shadow-xl hover:shadow-violet-300/60
        ...
        >
          <span className={`block w-6 h-0.5 bg-violet-600 rounded-full transition-all duration-300 origin-center
                           ${menuOpen ? 'translate-y-[7px] rotate-45' : ''}`} />
          <span className={`block w-6 h-0.5 bg-violet-600 rounded-full transition-all duration-300
      ...
      {/* ── Mobile Drawer ── */}
      {menuOpen && (
        <div className="lg:hidden absolute top-full right-0 w-72 h-screen
