...
export default function Services() {
  ...
  useEffect(() => {
    ...
    [headerRef, ctaRef, ...cardRefs.current]
      .map((r) => r?.current ?? r)
  ...
  return (
    <section id="services" className="py-36 relative overflow-hidden">
      {/* BG blobs */}
      <div
        className="pointer-events-none absolute -top-24 -right-48 w-[600px] h-[600px] rounded-full opacity-50"
        style={{
          background:
            "radial-gradient(circle, rgba(23, 105, 255,.07), transparent 70%)",
          filter: "blur(80px)",
        }}
      />
      <div
        className="pointer-events-none absolute bottom-0 -left-36 w-[500px] h-[500px] rounded-full opacity-40"
        style={{
          background:
            "radial-gradient(circle, rgba(255, 180, 0,.06), transparent 70%)",
          filter: "blur(80px)",
        }}
      />
      ...
      <div className="max-w-7xl mx-auto px-8">
        ...
        >
          ...
          <p className="text-[1.125rem] font-sans font-medium text-text-sec leading-[1.8] max-w-[520px]">
            From single symbols to complete game art packages — premium quality,
            every time.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-14">
          {servicesData.map((s, i) => (
            ...
            >
              ...
              <div
                className="absolute -bottom-10 -right-10 w-36 h-36 rounded-full pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-500"
                style={{
                  background:
                    "radial-gradient(circle, rgba(23, 105, 255,.12), transparent 70%)",
                }}
              />
              ...
              <div className="flex items-center justify-between">
                <div
                  className="w-14 h-14 rounded-2xl bg-primary-soft border border-border-subtle flex items-center justify-center text-[1.4rem]
              ...
              <h3 className="text-[1.25rem] font-subheading font-[600] text-text-main">
                {s.title}
              </h3>
              <p className="text-[.95rem] font-sans font-medium text-text-sec leading-[1.8] flex-1">
                {s.description}
              </p>
